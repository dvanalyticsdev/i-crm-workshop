...
test("incoming Meta leads route admission traffic into Main Admission Leads", () => {
  ...
  assert.match(mainAdmission, /<option value="Unassigned" \$\{filter\.counselor === "Unassigned" \? "selected" : ""\}>Unassigned<\/option>/);
...
test("Integration exposes lead inflow reporting with source campaign and duplicate metrics", () => {
  ...
  assert.match(server, /workshopNameFilter/);
  assert.match(server, /workshopDateFilter/);
  assert.match(server, /courseNameFilter/);
  assert.match(server, /workshopName: 1/);
  assert.match(server, /workshopDateLabel: 1/);
  assert.match(server, /courseName: 1/);
  ...
  assert.match(leadInflowHtml, /inflowWorkshopNameFilter/);
  assert.match(leadInflowHtml, /inflowWorkshopDateFilter/);
  assert.match(leadInflowHtml, /inflowCourseNameFilter/);
  ...
  assert.match(leadInflow, /url\.searchParams\.set\("workshopName", workshopNameFilter\.value \|\| "all"\)/);
  assert.match(leadInflow, /url\.searchParams\.set\("workshopDate", workshopDateFilter\.value \|\| "all"\)/);
  assert.match(leadInflow, /url\.searchParams\.set\("courseName", courseNameFilter\.value \|\| "all"\)/);
  assert.match(leadInflow, /payload\.filters\?\.workshopNames/);
  assert.match(leadInflow, /payload\.filters\?\.workshopDates/);
  assert.match(leadInflow, /payload\.filters\?\.courseNames/);
...
test("main admission page uses scoped loading with full-state fallback", () => {
  ...
  assert.doesNotMatch(mainAdmission, /function shouldBypassScopedMainAdmissionLoad/);
  assert.doesNotMatch(mainAdmission, /ensureFullStateForScopedBypass/);
  assert.match(getNamedFunctionSource(mainAdmission, "getScopedMainAdmissionFilterPayload"), /"assignedTimeline"/);
  assert.match(getNamedFunctionSource(mainAdmission, "getScopedMainAdmissionFilterPayload"), /"leadOwner"/);
  assert.doesNotMatch(getNamedFunctionSource(mainAdmission, "loadScopedMainAdmissionLeads"), /full-state-bypass|shouldBypassScopedMainAdmissionLoad/);
  assert.doesNotMatch(getNamedFunctionSource(mainAdmission, "shouldUseScopedServerPage"), /shouldBypassScopedMainAdmissionLoad/);
...
test("performance logs dashboard is super admin only", () => {
  ...
  assert.match(server, /app\.delete\("\/api\/performance-logs"/);
...
test("performance logs capture user-facing page, role, section, subsection, and phase context", () => {
  ...
  assert.match(server, /pageRoles: pageRoleRows/);
  assert.match(server, /roles: roleRows\.slice/);
  assert.match(server, /activities: activityRows/);
  assert.match(server, /fastest: allSpeedRows/);
  assert.match(server, /medium: allSpeedRows/);
  assert.match(server, /slowest: allSpeedRows/);
  ...
  assert.match(server, /function getPerformanceScore/);
  assert.match(server, /overallScore/);
  ...
  assert.match(performanceHtml, /performancePageRoleTable/);
  assert.match(performanceHtml, /overallPerformanceTrendChart/);
  assert.match(performanceHtml, /performanceActivityTable/);
  assert.match(performanceHtml, /performanceFastestTable/);
  assert.match(performanceHtml, /performanceMediumTable/);
  assert.match(performanceHtml, /performanceSlowestTable/);
  assert.match(performanceHtml, /clearPerformanceLogsBtn/);
  assert.match(performanceHtml, /Last 6 hours/);
  assert.match(performanceJs, /Overall Performance/);
  assert.match(performanceJs, /Average Page Loading/);
  assert.match(performanceJs, /Success vs Failure/);
  assert.match(performanceJs, /summary\.pageRoles/);
  assert.match(performanceJs, /summary\.activities/);
  assert.match(performanceJs, /summary\.fastest/);
  assert.match(performanceJs, /performance-name-cell/);
  assert.match(performanceJs, /function drawPerformanceTrend/);
